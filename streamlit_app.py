...
""")
    ...
    if chain_df.empty:
        ...
        dfc = (chain_df
               .query("chain == @chain_sel and metric == @metric_sel")
               .set_index("date")
               .sort_index())
        ...
        if dfc.empty:
            ...
            st.plotly_chart(
                px.line(dfc, y="value", title=f"{metric_sel} over time"),
    ...
    if not has_prophet:
        ...
        if not price_cols:
            ...
            fig.add_trace(go.Scatter(x=merged.ds, y=merged.yhat_upper, mode="lines",
                                     line=dict(width=0), showlegend=False))
            fig.add_trace(go.Scatter(x=merged.ds, y=merged.yhat_lower, mode="lines",
    ...
    if chain_df.empty:
        ...
        # slider only uses Python datetime
        dmin_ts = chain_df.date.min().to_pydatetime()
        dmax_ts = chain_df.date.max().to_pydatetime()
        ...
        if not pivot.empty:
            ...
            st.info("No data in selected range to summarize.")
